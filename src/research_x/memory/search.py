...
if TYPE_CHECKING:
    from research_x.memory.objective_routes import RouteAwareRetrievalPlan
...
class MemorySearchResult:
    ...
    output_mode: str = "explore"
    participation_snapshot: dict[str, Any] | None = None
...
def search_memory(
    ...
    *,
    ...
    if route:
        ...
        route_plan = plan_route_aware_retrieval(query, requested_route=route)
    semantic_execution = _semantic_execution_for_route(
        route_plan,
        db_path=path,
    ...
    effective_semantic_provider = (
        semantic_provider if semantic_execution["execute_semantic"] else None
    )
    effective_semantic_profile = str(
        semantic_execution.get("effective_embedding_profile")
        or semantic_execution.get("embedding_profile")
        or semantic_profile
        or ""
    ) or None
    effective_semantic_space_id = str(
        semantic_execution.get("effective_semantic_space_id")
        or semantic_execution.get("semantic_space_id")
    ...
    with sqlite3.connect(path, timeout=60) as conn:
        ...
        if conn.in_transaction:
            # Schema setup may upsert/backfill rows; release the write lock before
            # semantic search opens its own SQLite connection.
            conn.commit()
        ...
        if effective_semantic_provider:
            semantic_hits = _semantic_hits(
                path,
                query,
                space_id=effective_semantic_space_id,
                provider=None
                if effective_semantic_provider == "auto"
                else effective_semantic_provider,
                model=semantic_model,
                dimensions=semantic_dimensions,
                embedding_profile=effective_semantic_profile,
        ...
        participation_lookup = _participation_lookup_for_candidates(conn, candidates)
    ...
    final_results = tuple(results[:resolved_limit])
    if (
        explain_filters
        or intent
        or author_id
        or bookmark_owner_account_id
        or source_kind
        or ownership_kind
        or content_role
        or relation_role
        ...
        or filter_space_id
        or require_projections
    ):
        from research_x.memory.embedding_input import apply_metadata_filters
        ...
        final_results = filtered.results[:resolved_limit]
    ...
    *,
    ...
    by_doc = {
        hit.doc_id: SemanticScore(
            ...
            source_doc_hash=hit.source_doc_hash,
            embedded_text_hash=hit.embedded_text_hash,
            generated_at=hit.generated_at,
            stale_status=hit.stale_status,
            projection_generation_id=hit.projection_generation_id,
            projection_hash=hit.projection_hash,
            projection_status=hit.projection_status,
        )
        for hit in hits
    }
...
def format_search_results(
    ...
    *,
    ...
    route_plan: RouteAwareRetrievalPlan | None = None,
    filter_explanation: dict[str, Any] | None = None,
) -> str:
    if json_output:
        payload: Any = results_as_dicts(results)
        if route_plan is not None or filter_explanation is not None:
            payload = {
                "route_plan": route_plan.as_dict() if route_plan is not None else None,
                "filter_audit": filter_explanation,
                "results": payload,
            }
    ...
    if route_plan is not None:
        engine_text = ", ".join(choice.engine_run_id for choice in route_plan.engine_choices)
        route_payload = route_plan.as_dict()
        local_baseline = ", ".join(
            route_payload["search_execution_contract"]["local_baseline_engines"]
        )
        blocks.append(
            "\n".join(
                (
                    f"route: {route_plan.route_tag}",
                    f"route_choices(advisory): {engine_text}",
                    f"local_baseline(executed): {local_baseline}",
                    "fusion: separate engine runs; raw semantic score fusion disabled",
                )
            )
        )
    if filter_explanation is not None:
        blocks.append(
            "\n".join(
                (
                    f"intent: {filter_explanation.get('intent')}",
                    "filters: "
                    + json.dumps(
                        filter_explanation.get("applied_filters") or {},
                        ensure_ascii=False,
                        sort_keys=True,
                    ),
                    "excluded: "
                    + json.dumps(
                        filter_explanation.get("excluded_candidate_counts") or {},
                        ensure_ascii=False,
                        sort_keys=True,
                    ),
                )
        ...
        blocks.append(
            "\n".join(
                [
                    ...
                    "source restoration/context chunk required for citation",
                ]
            )
        )
    return "\n\n".join(blocks)
...
def _semantic_execution_for_route(
    route_plan: RouteAwareRetrievalPlan | None,
    *,
    db_path: Path | None = None,
    semantic_provider: str | None,
    semantic_space_id: str | None,
    semantic_profile: str | None,
) -> dict[str, Any]:
    if route_plan is None:
        return {
            "status": "legacy_semantic_options",
            "execute_semantic": bool(semantic_provider),
            "effective_semantic_space_id": semantic_space_id,
            "effective_embedding_profile": semantic_profile,
            "embedding_profile": semantic_profile,
            "requires_explicit_single_space": False,
        }
    route_semantic_choices = tuple(
        choice
        for choice in route_plan.semantic_engine_choices
        if choice.provider_role == "text_embedding"
    )
    executable_choices = tuple(
        choice for choice in route_semantic_choices if _route_semantic_choice_is_executable(choice)
    )
    blocked_choices = tuple(
        choice
        for choice in route_semantic_choices
        if not _route_semantic_choice_is_executable(choice)
    )
    base = {
        "route_tag": route_plan.route_tag,
        "route_semantic_space_ids": tuple(
            choice.semantic_space_id
            for choice in route_semantic_choices
            if choice.semantic_space_id
        ),
        "semantic_space_ids": tuple(
            choice.semantic_space_id
            for choice in route_semantic_choices
            if choice.semantic_space_id
        ),
        "executable_semantic_space_ids": tuple(
            choice.semantic_space_id for choice in executable_choices if choice.semantic_space_id
        ),
        "blocked_semantic_space_ids": tuple(
            choice.semantic_space_id for choice in blocked_choices if choice.semantic_space_id
        ),
        "raw_score_fusion_allowed": False,
        "multiple_semantic_spaces": "separate_engine_runs_only",
    }
    explicit_choice = _explicit_route_semantic_choice(
        route_plan,
        route_semantic_choices=route_semantic_choices,
        db_path=db_path,
        semantic_space_id=semantic_space_id,
        semantic_profile=semantic_profile,
    )
    if not semantic_provider:
        return {
            **base,
            ...
            "requires_explicit_single_space": len(executable_choices) > 1,
            "explicit_semantic_choice_validated": explicit_choice is not None,
        }
    if explicit_choice is not None:
        status = "explicit_space_id" if semantic_space_id else "explicit_embedding_profile"
        effective_space_id = semantic_space_id if semantic_space_id else None
        embedding_profile = explicit_choice.embedding_profile
        return {
            **base,
            "status": status,
            "execute_semantic": True,
            "route_semantic_space_id": explicit_choice.semantic_space_id,
            "semantic_space_id": effective_space_id,
            "effective_semantic_space_id": effective_space_id,
            "embedding_profile": embedding_profile,
            "effective_embedding_profile": embedding_profile,
            "requires_explicit_single_space": False,
        }
    if len(executable_choices) == 1:
        choice = executable_choices[0]
        return {
            **base,
            "status": "route_default_single_semantic_profile",
            "execute_semantic": True,
            "route_semantic_space_id": choice.semantic_space_id,
            "embedding_profile": choice.embedding_profile,
            "effective_embedding_profile": choice.embedding_profile,
            "requires_explicit_single_space": False,
        }
    if len(executable_choices) > 1:
        return {
            **base,
            "status": "skipped_requires_explicit_single_space",
            "execute_semantic": False,
            "requires_explicit_single_space": True,
            "reason": (
                "route selected multiple semantic spaces; pass --semantic-space-id "
                "or --semantic-profile to run one typed vector space"
            ),
        }
    return {
        **base,
        "status": "skipped_no_executable_route_semantic_choice",
        "execute_semantic": False,
        "requires_explicit_single_space": False,
        "reason": "route semantic choices are provider-gated or unavailable locally",
    }


def _explicit_route_semantic_choice(
    route_plan: RouteAwareRetrievalPlan,
    *,
    route_semantic_choices: tuple[Any, ...],
    db_path: Path | None,
    semantic_space_id: str | None,
    semantic_profile: str | None,
) -> Any | None:
    if semantic_space_id:
        choice = _semantic_choice_for_space_id(
            route_semantic_choices,
            db_path=db_path,
            semantic_space_id=semantic_space_id,
        )
        if choice is None:
            raise ValueError(
                f"semantic space_id {semantic_space_id!r} is outside route "
                f"{route_plan.route_tag!r} semantic choices: "
                f"{_route_semantic_choice_summary(route_semantic_choices)}"
            )
        if semantic_profile and semantic_profile != choice.embedding_profile:
            raise ValueError(
                f"semantic profile {semantic_profile!r} conflicts with semantic "
                f"space_id {semantic_space_id!r} for route {route_plan.route_tag!r}; "
                f"expected profile {choice.embedding_profile!r}"
            )
        _require_executable_route_semantic_choice(
            choice,
            route_tag=route_plan.route_tag,
            label="semantic space_id",
            value=semantic_space_id,
        )
        return choice
    if semantic_profile:
        choice = _semantic_choice_for_profile(route_semantic_choices, semantic_profile)
        if choice is None:
            raise ValueError(
                f"semantic profile {semantic_profile!r} is outside route "
                f"{route_plan.route_tag!r} semantic choices: "
                f"{_route_semantic_choice_summary(route_semantic_choices)}"
            )
        _require_executable_route_semantic_choice(
...
def _semantic_choice_for_space_id(
    route_semantic_choices: tuple[Any, ...],
    *,
    db_path: Path | None,
    ...
    for choice in route_semantic_choices:
        if choice.semantic_space_id == semantic_space_id:
            return choice
    stored_profile = _stored_semantic_space_profile(db_path, semantic_space_id)
    if stored_profile:
        return _semantic_choice_for_profile(route_semantic_choices, stored_profile)
    return None


def _semantic_choice_for_profile(
    route_semantic_choices: tuple[Any, ...],
    profile: str,
) -> Any | None:
    for choice in route_semantic_choices:
        if choice.embedding_profile == profile:
            return choice
    return None


def _stored_semantic_space_profile(db_path: Path | None, semantic_space_id: str) -> str | None:
    if db_path is None:
        return None
    with sqlite3.connect(db_path, timeout=60) as conn:
        ...
        if row:
            return str(row["embedding_profile"])
        row = conn.execute(
            """
            SELECT embedding_profile
            FROM memory_embeddings
            WHERE space_id = ?
            GROUP BY embedding_profile
...
def _require_executable_route_semantic_choice(
    choice: Any,
    *,
    route_tag: str,
    label: str,
    ...
    if _route_semantic_choice_is_executable(choice):
        return
    raise ValueError(
        f"{label} {value!r} for route {route_tag!r} is provider-gated or "
        f"unavailable for local route-mode execution"
    )


def _route_semantic_choice_is_executable(choice: Any) -> bool:
    return (
...
def _route_semantic_choice_summary(route_semantic_choices: tuple[Any, ...]) -> str:
    values = []
    for choice in route_semantic_choices:
        values.append(
            f"{choice.semantic_space_id or '-'}"
            f"/{choice.embedding_profile or '-'}"
            f"/{choice.local_execution}"
        )
    ...
    *,
    ...
    sql = f"""
        SELECT
            ...
            d.source_doc_hash, d.source_refs_json,
...
def _metadata_search(
    ...
    *,
    ...
    if plan.wants_cross_account and _table_exists(conn, "account_bookmarks"):
        clauses.append(
            """
...
def _relation_expansion_search(
    ...
    *,
    ...
    if not doc_ids:
        return []
    placeholders = ",".join("?" for _ in doc_ids)
    filters, params = _relation_expansion_filters(
        doc_type=doc_type,
        account=account,
        has_account_bookmarks=_table_exists(conn, "account_bookmarks"),
    )
    rows = conn.execute(
        f"""
        WITH related AS (
        ...
        row["_engine_contributions"] = [
            {
                ...
                "space_id": semantic["space_id"],
                "model": semantic["model"],
                "dimensions": semantic["dimensions"],
                "embedding_profile": semantic["embedding_profile"],
                "text_template_version": semantic["text_template_version"],
                "source_doc_hash": semantic["source_doc_hash"],
                "embedded_text_hash": semantic["embedded_text_hash"],
                "generated_at": semantic["generated_at"],
                "stale_status": semantic["stale_status"],
                "projection_generation_id": semantic["projection_generation_id"],
                "projection_hash": semantic["projection_hash"],
                "projection_status": semantic["projection_status"],
...
def _semantic_signal_policy(provider: str) -> dict[str, Any]:
    return {
        **embedding_provider_signal_policy(provider),
        "candidate_only": True,
        "raw_score_fusion_allowed": False,
        "score_fusion_scope": "single_semantic_space",
    }
...
def _dedupe_engine_contributions(values: list[Any]) -> list[dict[str, Any]]:
    ...
    for value in values:
        ...
        key = (
            ...
            str(value.get("space_id") or "") or None,
        ...
        *contributions,
        {
            ...
            "space_id": semantic_hit.space_id,
            ...
            "profile": semantic_hit.embedding_profile,
            "source_doc_hash": semantic_hit.source_doc_hash,
            "embedded_text_hash": semantic_hit.embedded_text_hash,
            "generated_at": semantic_hit.generated_at,
            "stale_status": semantic_hit.stale_status,
            "projection_generation_id": semantic_hit.projection_generation_id,
            "projection_hash": semantic_hit.projection_hash,
            "projection_status": semantic_hit.projection_status,
...
def _result_from_candidate(
    ...
    *,
    ...
    participation_lookup: dict[str, dict[str, Any]],
    route_plan: RouteAwareRetrievalPlan | None = None,
    ...
    components = {
        ...
        "semantic": _semantic_score_component(
            semantic_hit,
            semantic_weight=semantic_weight,
            route_plan=route_plan,
        ),
    ...
    if route_plan is not None:
        route_metadata = route_plan.compact_search_metadata()
        metadata["route_plan"] = route_metadata
        metadata["route_semantic_execution"] = semantic_execution or {}
        metadata["route_execution"] = {
            "advisory_engine_choice_ids": tuple(
                choice.engine_run_id for choice in route_plan.engine_choices
            ),
            "local_baseline_engines": tuple(route_metadata["local_baseline_engines"]),
            "executed_engines_for_result": tuple(
                sorted(
                    {
                        str(contribution.get("engine"))
                        for contribution in engine_contributions
                        if contribution.get("engine")
                    }
                )
            ),
            "semantic_execution_status": (semantic_execution or {}).get("status"),
        }
    ...
    if semantic_hit:
        metadata["semantic"] = {
            ...
            "weight": 0.0 if route_plan is not None else semantic_weight,
            "legacy_raw_score_component": route_plan is None,
            **_semantic_signal_policy(semantic_hit.provider),
        }
    ...
    primary_source_ref = source_refs[0] if source_refs else None
    participation_snapshot = _participation_snapshot_for_candidate(
    ...
    return MemorySearchResult(
        ...
        authority_level="source_backed" if source_refs else "candidate",
        source_status="available" if source_refs else "unknown",
        output_mode="explore",
        participation_snapshot=participation_snapshot,
        risk_flags=("search_result_not_evidence",),
    )


def _semantic_score_component(
    semantic_hit: SemanticHit | SemanticScore | None,
    *,
    semantic_weight: float,
    route_plan: RouteAwareRetrievalPlan | None,
    ...
    if route_plan is not None:
        return 0.0
    return max(0.0, _safe_float(semantic_hit.similarity)) * semantic_weight
...
def _relation_expansion_filters(
    *,
    ...
    if doc_type:
        parts.append("AND d.doc_type = ?")
        params.append(doc_type)
    if account and has_account_bookmarks:
        parts.append(
            """
            AND (
                d.account_id = ?
...
def _loads_json_list(value: Any) -> tuple[str, ...]:
    if not value:
        return ()
...
def _source_refs_from_row(row: dict[str, Any]) -> tuple[str, ...]:
    refs = _loads_json_list(row.get("source_refs_json"))
    if refs:
        return refs
    tweet_id = row.get("source_tweet_id")
    if tweet_id:
        return (f"x:tweet:{tweet_id}",)
    return ()


def _participation_lookup_for_candidates(
    conn: sqlite3.Connection,
    candidates: list[dict[str, Any]],
    ...
    for candidate in candidates:
        source_refs.update(_source_refs_from_row(candidate))
    ...
    if source_refs:
        ...
        rows = conn.execute(
            f"""
            SELECT source_ref, artifact_id, output_mode, policy_version,
                   can_search, can_explore, can_use_in_working_note,
                   can_use_as_evidence, can_use_in_answer,
                   can_trigger_external_fetch, reason, decided_at
            FROM memory_participation_decisions
            WHERE source_ref IN ({placeholders})
            """,
            tuple(sorted(source_refs)),
        ).fetchall()
        _merge_participation_rows(lookup, rows)
    if artifact_ids:
...
def _merge_participation_rows(
    lookup: dict[str, dict[str, Any]],
    rows: list[sqlite3.Row],
) -> None:
    for row in rows:
        source_ref = _string_or_none(row["source_ref"])
        artifact_id = _string_or_none(row["artifact_id"])
        keys = []
        if source_ref:
            keys.append(f"source:{source_ref}")
        if artifact_id:
            keys.append(f"artifact:{artifact_id}")
        for key in keys:
            snapshot = lookup.setdefault(
                key,
                {
                    "policy_source": "memory_participation_decisions",
                    "output_modes": [],
                    "policy_versions": [],
                    "reasons": [],
                    "can_search": False,
                    "can_explore": False,
                    "can_use_in_working_note": False,
                    "can_use_as_evidence": False,
                    "can_use_in_answer": False,
                    "can_trigger_external_fetch": False,
                },
            )
            output_mode = str(row["output_mode"] or "")
            if output_mode and output_mode not in snapshot["output_modes"]:
                snapshot["output_modes"].append(output_mode)
            policy_version = str(row["policy_version"] or "")
            if policy_version and policy_version not in snapshot["policy_versions"]:
                snapshot["policy_versions"].append(policy_version)
            reason = str(row["reason"] or "")
            if reason and reason not in snapshot["reasons"]:
            ...
            for field in (
                ...
                snapshot[field] = bool(snapshot[field] or row[field])


def _participation_snapshot_for_candidate(
    *,
    source_refs: tuple[str, ...],
    ...
    if artifact_id:
        snapshot = participation_lookup.get(f"artifact:{artifact_id}")
        if snapshot:
            snapshots.append(snapshot)
    for source_ref in source_refs:
        snapshot = participation_lookup.get(f"source:{source_ref}")
        if snapshot:
            snapshots.append(snapshot)
    if snapshots:
        combined = {
            ...
            "search_result_can_support_answer": False,
            "policy_source": "memory_participation_decisions",
            "output_modes": sorted(
                {
                    mode
                    for snapshot in snapshots
                    for mode in snapshot.get("output_modes", [])
                }
            ),
            "reasons": sorted(
                {
        ...
        for field in (
            ...
            combined[field] = any(snapshot.get(field) is True for snapshot in snapshots)
        return combined
    return {
        "source_ref": primary_source_ref,
        "source_refs": list(source_refs),
        "artifact_id": artifact_id,
        "search_result_can_support_answer": False,
        "policy_source": "default_no_decision",
        "can_search": True,
        "can_explore": True,
        "can_use_in_working_note": True,
        "can_use_as_evidence": False,
        "can_use_in_answer": False,
        "can_trigger_external_fetch": False,
        "reasons": ["no_memory_participation_decision"],
    }
...
def _bookmark_account_counts(
    ...
    if not tweet_ids or not _table_exists(conn, "account_bookmarks"):
        return {}
