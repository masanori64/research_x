...
| S51 | `marp-slidev` | https://tech-lab.sios.jp/archives/52940, https://marp.app/, and https://sli.dev/ | Presentation-artifact workflow reference. Marp is already the owned deck assembly boundary; local visual QA evaluation is project-owned for already-rendered artifacts. Slidev runtime, Playwright capture, ppt-master generation, dependency/install, browser automation, and generated-deck evidence promotion remain disabled. |
...
| S56 | `agent-guardrail-design` | https://qiita.com/ryuichi000persol/items/27789cbca88bd4bf11e0 and https://zenn.dev/nrs/articles/e4a2ae8a9fb785 | Agent safety and loop-control design references. Use only as eval/reference material for local trace/contract boundary checks around tool boundaries, forbidden external actions, provider gates, parameter validation, and bounded stop reasons; no agent framework, dependency, MCP/plugin, browser automation, runtime route, or prompt-only safety claim is adopted. |
...
| S58 | `aws-summit-japan-2026-event-login-boundary` | https://aws.amazon.com/jp/events/summits/japan/ and https://registration.awsevents.com/flow/awsevents/summitjap26/genreg/login | Event/login source-boundary reference. The public event page, registration portal, login-only attendee materials, livestream archives, and user-provided exports are separate restoration states. No login, browser automation, scraping, provider Reader, hidden API, source-bundle promotion, citation readiness, or answer support is authorized from the registration locator or event metadata alone. |
