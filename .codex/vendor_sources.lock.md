...
| S34 | `pdgkit` | https://github.com/shibayamalicht/pdgkit and npm `@shibayama/pdgkit` | Pinned body-adoption canary for item 35 deterministic diagram/spec rendering. MIT license and npm version `0.1.2` checked 2026-06-23. Package lock is isolated under `tools/pdgkit_canary/`; real `pdgkit validate` and SVG `pdgkit render` passed for `canaries/item-11-35-flow.pdg`, with deterministic SHA-256 `1023BB0031CC07D94150CCCAD13771A440C7F2094A430770E50FA65A4CB8B623`. Decision: LIMITED local `.pdg -> validate -> SVG` artifact lane only. No `pdgkit-mcp`, provider use, root dependency, global install, PNG/PDF/PPTX promotion, or evidence promotion. |
