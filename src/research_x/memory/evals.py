    ...
    *,
    ...
    with sqlite3.connect(path, timeout=60) as conn:
        ...
        for index, result in enumerate(results):
            suite = cases_path or "default"
            conn.execute(
                """
                INSERT INTO memory_eval_cases (
                    eval_case_id, suite, objective, expected_status,
                    required_source_ids_json, forbidden_source_ids_json,
                    route_tags_json, created_at, metadata_json
                )
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                ON CONFLICT(eval_case_id) DO UPDATE SET
                    ...
                    route_tags_json=excluded.route_tags_json,
                    metadata_json=excluded.metadata_json
                """,
                (
                    _eval_case_id(suite, index, result.query),
                    suite,
                    result.query,
                    None,
                    "[]",
                    ...
                    json.dumps(
                        {
                            ...
                            "retrieval_engines": result.retrieval_engines,
            ...
            conn.execute(
                ...
                INSERT INTO memory_eval_results (
                    ...
                    expected_route, stop_reason, hits, context_chunks,
                    workflow_id, context_run_id, first_doc_id, best_score,
                    matched_terms_json, retrieval_engines_json, source_kinds_json,
                    answer_status, answer_citations, notes_json, metadata_json,
                    created_at
                )
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
...
def _evaluate_case(
    ...
    answer_citations = len(_answer_or_context_citations(workflow)) if workflow.answer else 0
    ...
    if not hits:
        ...
        return MemoryEvalResult(
            ...
            context_run_id=(
                workflow.context_bundle.run_id if workflow.context_bundle is not None else None
            ),
            hits=0,
    ...
    if workflow.answer is not None:
        ...
        if hits and not _answer_or_context_citations(workflow):
...
def _eval_citations(workflow: MemoryWorkflow) -> tuple[CitationAnnotation, ...]:
    if workflow.answer is not None:
        return _answer_or_context_citations(workflow)
    if workflow.context_bundle is not None:
        return workflow.context_bundle.citation_annotations
    return ()


def _answer_or_context_citations(workflow: MemoryWorkflow) -> tuple[CitationAnnotation, ...]:
    if workflow.answer is None:
        return ()
    if workflow.answer.citation_annotations:
        ...
        sorted(
            {
                ...
                for citation in _answer_or_context_citations(workflow)
...
def _eval_case_id(suite: str, index: int, query: str) -> str:
    return _hash_id("memory-eval-case", suite, str(index), query)[:24]
