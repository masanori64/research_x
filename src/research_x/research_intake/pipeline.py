...
POLICY_CONTROLLED_ENABLEMENT = {"provider_execution_policy", "authorized_provider_policy"}
...
class FetchSnapshot:
    ...
    source_restore_ref: str | None
    storage_rights: str
    prompt_injection_review: str
    promotion_gate: str

    @property
    def source_bundle_ref(self) -> str | None:
        """Compatibility name for the same restorable source lineage pointer."""

        return self.source_restore_ref

    def as_dict(self) -> dict[str, Any]:
        data = asdict(self)
        data["source_bundle_ref"] = self.source_restore_ref
        return data
...
class DiscoveryRun:
    ...
    network_mode: str
    limit: int
    provider_policy_compliant: bool
...
def validate_registry(registry: SourceRegistry) -> list[str]:
    ...
    for source in registry.sources:
        ...
        if (
            source.source_type in PROVIDER_SOURCE_TYPES
            and _source_enabled(source)
            and source.enabled_when not in POLICY_CONTROLLED_ENABLEMENT
        ):
            errors.append(
                f"{source.source_id}: provider-backed sources require provider_execution_policy "
                "enablement for dry-run intake"
            )
        provider_policy_source = (
            source.source_type in PROVIDER_SOURCE_TYPES
            and source.enabled_when in POLICY_CONTROLLED_ENABLEMENT
        )
        if source.policy.allow_network and not provider_policy_source:
            errors.append(f"{source.source_id}: policy.allow_network must be false")
        if source.policy.allow_provider and not provider_policy_source:
            errors.append(f"{source.source_id}: policy.allow_provider must be false")
        allowed_fetch_modes = (
            {"metadata_only", "authorized_candidate_fetch"}
            if provider_policy_source
            else {"metadata_only"}
        )
        if source.policy.fetch_mode not in allowed_fetch_modes:
            errors.append(
                f"{source.source_id}: policy.fetch_mode must be one of "
                f"{sorted(allowed_fetch_modes)}"
            )
        ...
        if source.source_type in PROVIDER_SOURCE_TYPES:
            skipped.append(
                SkippedSource(
                    source_id=source.source_id,
                    source_type=source.source_type,
                    reason="provider_policy_required",
                )
            )
            continue
...
def validate_run(run: DiscoveryRun | dict[str, Any]) -> list[str]:
    ...
    if data.get("provider_policy_compliant") is not True:
        errors.append("provider_policy_compliant must be true")
    ...
    for snapshot in data.get("snapshots", []):
        ...
        if snapshot.get("source_restore_ref") is not None:
            errors.append(f"{snapshot.get('snapshot_id')}: source_restore_ref must be null")
    return errors
...
def _risk_flags(source: SourceSubscription) -> tuple[str, ...]:
    ...
    if source.source_type == "local_note":
        flags.append("local_note_not_source_restore")
    return tuple(flags)
