...
def _validate_source_artifacts(view_id: str, value: object) -> list[str]:
    ...
    for index, item in enumerate(value, start=1):
        ...
        artifact_kind = str(item.get("artifact_kind", "control_artifact"))
        if artifact_role != "control_state" or authority_level != "navigation_signal":
            errors.append(
                f"{prefix}: control source artifact must remain "
                "artifact_role=control_state and authority_level=navigation_signal"
            )
        if evidence_role in EVIDENCE_ROLES or evidence_status in EVIDENCE_ROLES:
            errors.append(
                f"{prefix}: {artifact_kind} cannot be evidence, citation, "
                "or answer support"
            )
        if item.get("can_use_as_evidence") is True or item.get("can_use_in_answer") is True:
            errors.append(
                f"{prefix}: participation flags must not grant evidence or answer use"
            )
        participation = item.get("participation_decision")
        if isinstance(participation, Mapping) and (
            participation.get("can_use_as_evidence") is True
            or participation.get("can_use_in_answer") is True
        ):
            errors.append(
                f"{prefix}: participation_decision must not grant evidence or answer use"
            )
