...
def test_run_pipeline_treats_cursor_exhaustion_as_complete(tmp_path, monkeypatch) -> None:
    ...
    results = run_pipeline(
        ...
        storage_state=tmp_path / "missing_state.json",
        min_successful_providers=1,
        require_cursor_exhaustion_for_bookmarks=True,
    )

    assert results[0].status == PipelineStatus.OK
    assert results[0].metadata["provider_exhausted"] is True
    assert results[0].metadata["cursor_exhaustion_required"] is True
    assert results[0].metadata["cursor_exhaustion_met"] is True


def test_exhaustive_bookmarks_do_not_succeed_without_cursor_exhaustion(
    tmp_path,
    monkeypatch,
) -> None:
    now = utc_now()

    class LimitMetButNotExhaustedAdapter:
        adapter_id = "cursor"
            ...
            return FetchOutcome(
                ...
                items=(
                    XItem(
                        ...
                        url=None,
                        author=None,
                        text="limit is met but cursor is still open",
                        created_at=None,
                        observed_at=now,
                        raw={"bookmark_root": True},
                    ),
    ...
    monkeypatch.setattr(
        "research_x.pipeline.build_adapter",
        lambda config: LimitMetButNotExhaustedAdapter(config),
    )
    config = ExperimentConfig(
        name="cursor-required",
        targets=(AcquisitionTarget(TargetKind.BOOKMARKS, "me", limit=1),),
        adapters=(AdapterConfig("cursor"),),
    )
    ...
    assert results[0].status == PipelineStatus.PARTIAL
    assert results[0].metadata["cursor_exhaustion_required"] is True
    assert results[0].metadata["cursor_exhaustion_met"] is False


def test_pipeline_records_redacted_provider_run_metadata(tmp_path, monkeypatch) -> None:
    now = utc_now()
    ...
    monkeypatch.setattr(
        "research_x.pipeline.build_adapter",
        lambda config: MetadataAdapter(config),
    )
    config = ExperimentConfig(
        name="metadata",
        targets=(AcquisitionTarget(TargetKind.SEARCH, "hello", limit=1),),
        adapters=(AdapterConfig("metadata"),),
    )
    ...
    results = run_pipeline(
        ...
        min_successful_providers=1,
        account_id="acct",
        account_profile=AccountProfile(account_id="acct", screen_name="acct"),
    )

    metadata = results[0].metadata
    assert metadata["session"] == {
        "has_session": False,
        "session_source": "missing_storage_state",
        "storage_state_path": str(tmp_path / "missing_state.json"),
    }
    assert metadata["account"]["account_id"] == "acct"
    assert metadata["provider_attempts"][0]["provider_route"] == "metadata"
    assert metadata["provider_attempts"][0]["request_count"] == 3
    assert metadata["provider_attempts"][0]["page_count"] == 2
    assert metadata["rate_limited"] is True
    assert results[0].attempts[0].outcome.metadata["auth_token"] == "[redacted]"
