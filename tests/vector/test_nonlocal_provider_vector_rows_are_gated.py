...
def test_text_embedding_provider_records_text_embedding_role_with_authorized_transport(
    vector_db_path: Path,
    monkeypatch,
) -> None:
    called = False
    ...
    upsert_api_price(
        ...
        unit="call",
        usd_per_unit=0.0,
        source_url="fixture://text-embedding-provider-role",
        notes="provider-free monkeypatched fixture",
    )

    with api_budget_context(
        db_path=vector_db_path,
        run_id="authorized-text-embedding",
    ...
    status = api_budget_status(vector_db_path, run_id="authorized-text-embedding")
    event = status["recent_events"][0]
    assert event["provider_role"] == "text_embedding"
    assert event["operation"] == "embedding"
    assert event["metadata"]["provider_policy_status"] == "authorized_by_provider_policy"
...
def test_nonlocal_vector_projection_metadata_is_quarantined_and_search_gated(
    ...
    assert policy["production_eligible_reason"] == "provider_policy_required"
...
def test_nonlocal_provider_audit_and_benchmark_metadata_are_quarantined(
    ...
    assert benchmark.metadata["provider_policy"] == "provider_policy_controlled"
...
def _provider_quota_approval() -> dict[str, object]:
    return {
        ...
        "max_calls": 3,
        "max_cost_usd": 0.0,
        "price_source": "fixture://text-embedding-provider-role",
        "approved_scope": "*",
