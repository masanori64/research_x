...
def test_presentation_package_surface_is_limited_to_local_renderers() -> None:
...
def test_stage1_lockfile_pins_selected_packages() -> None:
    ...
    assert root["devDependencies"]["@marp-team/marp-cli"] == "4.4.1"
    assert root["devDependencies"]["@terrastruct/d2"] == "0.1.33"
    assert lock["packages"]["node_modules/@marp-team/marp-cli"]["version"] == "4.4.1"
    assert "node_modules/@mermaid-js/mermaid-cli" not in lock["packages"]
...
def test_presentation_config_names_d2_marp_and_stage2_sources() -> None:
    ...
    assert "slides_source: docs/presentation/deck.marp" in config
