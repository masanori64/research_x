...
PROVIDER_POLICY_REQUIRED_MATCH = "provider_execution_policy_required"


def test_budgeted_api_call_requires_provider_policy_without_context() -> None:
    with pytest.raises(
        RuntimeError,
        match=PROVIDER_POLICY_REQUIRED_MATCH,
    ), budgeted_api_call(
        provider="openai",
        model="gpt-4o-mini",
...
def test_json_provider_send_wrappers_require_provider_policy_before_http(
    ...
    with pytest.raises(RuntimeError, match=PROVIDER_POLICY_REQUIRED_MATCH):
...
def test_media_embedding_send_wrapper_requires_provider_policy_before_http(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    called = False
...
def test_reader_provider_requires_provider_policy_before_network_fetch(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    called = False
...
def test_ocr_provider_requires_provider_policy_before_http_send(tmp_path: Path) -> None:
    with pytest.raises(RuntimeError, match=PROVIDER_POLICY_REQUIRED_MATCH):
...
def test_classifier_provider_send_wrapper_requires_provider_policy_before_http(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    called = False
