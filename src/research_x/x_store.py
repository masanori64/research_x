...
class MediaDownloadPolicy(StrEnum):
    NO_MEDIA_DOWNLOAD = "no_media_download"
    METADATA_ONLY = "metadata_only"
    THUMBNAILS_ONLY = "thumbnails_only"
    FULL_MEDIA_ALLOWED = "full_media_allowed"
...
class XStoreSummary:
    ...
    media_download_policy: str = MediaDownloadPolicy.FULL_MEDIA_ALLOWED.value
...
def write_x_store_outputs(
    ...
    *,
    ...
    media_download_policy: str | MediaDownloadPolicy | None = None,
    ...
    resolved_media_policy = _resolve_media_download_policy(
    ...
    _apply_media_download_policy(
        media_rows,
        resolved_media_policy,
        media_dir=media_dir,
    )

    if resolved_media_policy in {
        MediaDownloadPolicy.FULL_MEDIA_ALLOWED,
        MediaDownloadPolicy.THUMBNAILS_ONLY,
    }:
        ...
        for index, row in enumerate(media_rows, start=1):
            if row.get("download_status") == "pending":
    ...
    _write_sqlite(
        ...
        account_id=account_id,
        collection_kind=collection_kind,
        media_download_policy=resolved_media_policy,
    ...
    summary = XStoreSummary(
        ...
        account_id=account_id,
        run_id=resolved_run_id,
        media_download_policy=resolved_media_policy.value,
    )
...
def _write_sqlite(
    ...
    *,
    ...
    account_id: str | None,
    collection_kind: str,
    media_download_policy: MediaDownloadPolicy,
    collection_rows: list[dict[str, Any]],
    bookmark_rows: list[dict[str, Any]],
        ...
        for row in media_rows:
            conn.execute(
                ...
                INSERT INTO media (
                    ...
                    media_download_policy, media_sha256
                )
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ON CONFLICT(media_id) DO UPDATE SET
                    ...
                    media_download_policy=excluded.media_download_policy,
...
def _ensure_schema(conn: sqlite3.Connection) -> None:
    ...
    _ensure_column(conn, "media", "media_download_policy", "TEXT")
    _ensure_column(conn, "media", "media_sha256", "TEXT")
...
def _resolve_media_download_policy(
    media_download_policy: str | MediaDownloadPolicy | None,
    *,
    download_media: bool,
) -> MediaDownloadPolicy:
    if media_download_policy is None:
        return (
            MediaDownloadPolicy.FULL_MEDIA_ALLOWED
            if download_media
            else MediaDownloadPolicy.METADATA_ONLY
        )
    if isinstance(media_download_policy, MediaDownloadPolicy):
        return media_download_policy
    return MediaDownloadPolicy(str(media_download_policy))


def _apply_media_download_policy(
    ...
    *,
    ...
    for row in media_rows:
        row["media_download_policy"] = policy.value
        if policy == MediaDownloadPolicy.NO_MEDIA_DOWNLOAD:
            row["download_status"] = "download_not_allowed"
        elif policy == MediaDownloadPolicy.METADATA_ONLY or (
            policy == MediaDownloadPolicy.THUMBNAILS_ONLY and not _is_thumbnail_media(row)
        ):
            row["download_status"] = "metadata_only"
    if policy == MediaDownloadPolicy.NO_MEDIA_DOWNLOAD:
        return
    if policy == MediaDownloadPolicy.METADATA_ONLY:
        return
    if not media_rows:
        return
    media_dir.mkdir(parents=True, exist_ok=True)
...
def _is_thumbnail_media(row: dict[str, Any]) -> bool:
    media_type = str(row.get("type") or "").lower()
    url = str(row.get("url") or "").lower()
    return "thumbnail" in media_type or "thumb" in url
...
def _provider_run_metadata(
    ...
    *,
    ...
    payload["account_profile"] = _account_profile_metadata(account_id, account_profile)
    payload["collection_kind"] = collection_kind
    payload["media_download_policy"] = media_download_policy.value
    return payload


def _account_profile_metadata(account_id: str | None, profile: Any | None) -> dict[str, Any]:
    if profile is None:
        return {"account_id": account_id}
    return {
        "account_id": getattr(profile, "account_id", account_id),
        "screen_name": getattr(profile, "screen_name", None),
        "user_id": getattr(profile, "user_id", None),
        "display_name": getattr(profile, "display_name", None),
        "url": getattr(profile, "url", None),
        "metadata": _sanitize_metadata(getattr(profile, "metadata", {}) or {}),
    }
...
def _download_media(
    ...
    *,
    ...
    if target_path.exists() and target_path.stat().st_size > 0:
        ...
        row["media_sha256"] = _sha256_file(target_path)
        return
    ...
    try:
        with urlopen(request, timeout=timeout_seconds) as response:  # noqa: S310
            ...
            row["media_sha256"] = hashlib.sha256(data).hexdigest()
