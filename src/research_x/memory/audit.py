...
def _embedding_specs(conn: sqlite3.Connection) -> list[dict[str, Any]]:
    rows = conn.execute(
        ...
        SELECT
            ...
            COUNT(*) AS rows,
            COUNT(DISTINCT e.generation_id) AS generation_count,
        ...
        GROUP BY
            ...
            e.text_template_version
        ORDER BY
            rows DESC, space_id, provider, model, dimensions,
...
def _invalid_json_counts(conn: sqlite3.Connection) -> dict[str, int]:
    field_specs = [
        ...
        ("memory_eval_cases", "required_source_ids_json"),
        ("memory_eval_cases", "forbidden_source_ids_json"),
        ("memory_eval_cases", "route_tags_json"),
        ("memory_eval_cases", "metadata_json"),
        ...
        ("memory_retrieval_engine_runs", "filters_json"),
        ("memory_retrieval_engine_runs", "metadata_json"),
        ("memory_retrieval_candidates", "metadata_json"),
        ("memory_fusion_runs", "input_engine_runs_json"),
        ("memory_fusion_runs", "engine_weights_json"),
        ("memory_fusion_runs", "metadata_json"),
        ("memory_rerank_runs", "metadata_json"),
        ("memory_restoration_attempts", "metadata_json"),
    ]
...
def _needs_review_suggested_local_check(suggested_action: str) -> str:
    if suggested_action == "add_or_repair_citation":
        return "inspect source restoration, context chunk, and citation annotation linkage"
    if suggested_action == "resolve_conflict":
        return "compare conflicting source restorations and rerun local retrieval eval"
    if suggested_action == "restore_source_then_regenerate":
        return (
            "restore source restoration and context chunks before any "
            "provider-gated regeneration"
        )
...
def _classify_warning(message: str) -> dict[str, Any]:
    ...
    elif message.startswith("provider embedding rows require ProviderExecutionPolicy"):
        code = "provider_embedding_rows_quarantined"
        severity = "expected"
        category = "provider_gated_expected"
        gate = "provider_execution_policy"
        blocks_local = False
...
def _warnings(
    *,
    ...
    if production_specs:
        warnings.append(
            "provider embedding rows require ProviderExecutionPolicy before production use; "
