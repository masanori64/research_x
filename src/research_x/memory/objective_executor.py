    ...
    *,
    ...
    for index, route_arm in enumerate(selected_routes):
        ...
        if _should_stop(plan, result):
            ...
            status = "ok" if _result_satisfies_mode(plan, result) else "needs_review"
            break
        if result.escalation_trigger and result.escalation_trigger not in selected_routes:
            ...
            # because provider-backed escalations require ProviderExecutionPolicy before execution.
    ...
    source_restore_restoration_failures = sum(
        int(result.output.get("source_restore_restoration_failures", 0))
    ...
    metadata = {
        ...
        "provider_execution_policy": (
            "provider arms require scoped ProviderExecutionPolicy; "
            "local/fake arms may run by default"
        ),
        ...
        "guarded_fusion": {
            ...
            "fusion_scope": "source_restore_restored_local_arms",
            "raw_score_averaging": False,
        },
        "fallback_routes_used": fallback_routes_used,
        "skipped_provider_roles": skipped_provider_roles,
        "source_restore_restoration_failures": source_restore_restoration_failures,
        "source_bundle_restoration_failures": source_restore_restoration_failures,
    }
...
def _execute_arm(
    ...
    *,
    ...
    if route_arm in PROVIDER_POLICY_ARMS:
        return _skipped_arm(route_arm, reason="provider_policy_required")
...
def _execute_media_arm(
    ...
    *,
    ...
    content_hits = tuple(hit for hit in ocr_hits if _ocr_hit_has_content_evidence(hit))
    if content_hits:
        output = {
...
def _ocr_hit_has_content_evidence(hit: dict[str, Any]) -> bool:
    source_view = hit.get("source_view") or hit.get("bundle") or {}
    return isinstance(source_view, dict) and bool(source_view.get("media_content_evidence"))
...
def _result_from_context_bundle(
    ...
    return ObjectiveRouteArmResult(
        ...
        output={
            ...
            "source_restore_restoration_failures": _source_restore_restoration_failures(
                bundle.retrieved_hits
            ),
            "source_bundle_restoration_failures": _source_restore_restoration_failures(
                bundle.retrieved_hits
            ),
        },
    )
...
def _source_restore_restoration_failures(hits: list[dict[str, Any]]) -> int:
    failures = 0
    for hit in hits:
        ...
        if isinstance(metadata, dict) and metadata.get("source_restore_error"):
            failures += 1
    return failures
...
def _skipped_arm(route_arm: str, *, reason: str) -> ObjectiveRouteArmResult:
    return ObjectiveRouteArmResult(
        ...
        provider_quota_skipped=reason == "provider_policy_required",
...
def _should_stop(plan: ObjectiveRoutePlan, result: ObjectiveRouteArmResult) -> bool:
    ...
    if result.stop_condition == "external_context_needed":
        return False
    if not _result_satisfies_mode(plan, result):
        return False
...
def _result_satisfies_mode(
    plan: ObjectiveRoutePlan,
    result: ObjectiveRouteArmResult,
) -> bool:
    mode = normalize_output_mode(plan.output_mode)
    if mode in {
        OutputMode.EXPLORE,
        OutputMode.COLLECT,
    ...
    if mode is OutputMode.EVIDENCE_PACKAGE:
        return result.citation_count > 0
    # ObjectiveRouteExecution retrieves and audits evidence; it does not build
    # the final answer assertion. Answer mode must continue to the answer
    # boundary even when citation-ready evidence has been found.
    return False
