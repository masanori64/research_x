...
DEFAULT_VENDOR_SOURCE_LOCK_PATH = Path("control") / "vendor_sources.lock.toml"
...
PROVIDER_POLICY_TOKENS = {
    "provider",
    "effective runtime control state",
    "api budget guard",
    "source restoration",
}
PROVIDER_POLICY_SHAPES = {
    "approved_experimental",
    "implemented_experimental",
    "eval_passed",
    "production_candidate",
...
def _validate_policy(policy: dict[str, Any], errors: list[str]) -> None:
    expected = {
        ...
        "runtime_permission_control_required": True,
        "api_budget_guard_required": True,
        "paid_quota_provider_pause_report_required": False,
        "research_x_is_codex_foundation": False,
        "codex_foundation_home": "${CODEX_HOME}",
    }
...
def _validate_candidate(
    ...
    if candidate.provider_or_quota and candidate.adoption_shape not in PROVIDER_POLICY_SHAPES:
        errors.append(
            f"{candidate.name}: provider/quota candidate must use a provider policy shape"
        )
    if candidate.provider_or_quota:
        normalized_stop = candidate.stop_condition.casefold()
        missing_tokens = sorted(
            token
            for token in PROVIDER_POLICY_TOKENS
            if token not in normalized_stop
        )
        if missing_tokens:
            errors.append(
                f"{candidate.name}: provider policy condition missing: "
                + ", ".join(missing_tokens)
            )
    ...
    if candidate.status == "active_implementation" and candidate.enabled:
        errors.append(f"{candidate.name}: active implementation candidate cannot be enabled")
...
def _validate_source_lock(
    ...
    if not source_lock_path.exists():
        ...
        return
    with source_lock_path.open("rb") as handle:
