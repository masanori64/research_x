    ...
    *,
    ...
    try:
        ...
        bundle = build_evidence_bundle(
            ...
            semantic_space_id=spec.space_id if spec else None,
        ...
        arm = PortfolioArmResult(
            ...
            error=None,
            space_id=spec.space_id if spec else None,
        )
        return arm, hits
    except (FileNotFoundError, RuntimeError, ValueError) as exc:
    ...
    *,
    ...
    try:
        ...
        for result in report.results:
            ...
            metadata["rerank"] = {
                **result.as_dict(),
                "evidence_role": "ranking_candidate_signal",
    ...
    try:
        ...
        hits = _annotate_hits(
            _dedupe_raw_hits(hits)[:limit],
            {
                ...
                "citation_policy": "candidate_until_source_restore_context",
    ...
    try:
        ...
        hits = _annotate_hits(
            list(bundle.retrieved_hits),
            {
                ...
                "portfolio_non_vector_arm_aliases": ["source_restore_context"],
...
def _run_semantic_only_arm(
    ...
    *,
    ...
    resolved_space_id = spec.space_id or next(
        (hit.space_id for hit in semantic_hits if hit.space_id),
        None,
    )
    arm = PortfolioArmResult(
        name=name,
    ...
    *,
    ...
    for arm, hits in arm_payloads:
        ...
        for rank, hit in enumerate(hits, start=1):
            ...
            current_hit = dict(bucket["hit"])
            representative = current_hit
            if _prefer_representative(hit, current_hit, rank, int(bucket["best_rank"])):
                representative = hit
                bucket["best_rank"] = rank
            bucket["hit"] = _merge_candidate_signal_metadata(
...
def _raw_hit_is_source_restorable(hit: dict[str, Any]) -> bool:
    ...
    return not (isinstance(metadata, dict) and metadata.get("source_restore_error"))
...
def _merge_candidate_signal_metadata(
    *,
    ...
    for source in (current, candidate):
        ...
        if "rerank" in metadata and "rerank" not in merged_metadata:
            merged_metadata["rerank"] = metadata["rerank"]
    if merged_metadata:
        merged["metadata"] = merged_metadata

    merged_score_components = dict(merged.get("score_components") or {})
    for source in (current, candidate):
        score_components = (
            source.get("score_components")
            if isinstance(source.get("score_components"), dict)
            else {}
        )
        for key, value in score_components.items():
            if str(key).startswith("rerank:") and key not in merged_score_components:
                merged_score_components[key] = value
    if merged_score_components:
        merged["score_components"] = merged_score_components
    return merged
