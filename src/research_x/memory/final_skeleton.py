    ...
    *,
    ...
    with sqlite3.connect(path, timeout=60) as conn:
        ...
        if store:
            ...
            register_projection_lifecycle(path)

    notes = [
        "provider policy is required; this preflight only writes local artifacts",
    ...
    return FinalSkeletonPreflightReport(
        ...
        next_paid_gate=(
            "provider-backed semantic/rerank/reader/OCR/media build requires scoped "
            "ProviderExecutionPolicy and the paid/quota report pause"
        ),
        notes=tuple(notes),
    )
...
def format_final_skeleton_preflight(report: FinalSkeletonPreflightReport) -> str:
    lines = [
        ...
        f"provider_policy_required: {report.provider_policy_required}",
...
def _eval_gate_rows(
    *,
    ...
    statuses = {
        ...
        "answer_eval": "provider_policy_required_before_answer",
