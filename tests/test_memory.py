...
def test_memory_feedback_and_corpus2skill_export(tmp_path: Path) -> None:
    ...
    assert (
        advisory_manifest["agent_advisory"]["provider_quota"]
...
def test_memory_vector_backend_benchmark_blocks_non_local_provider(tmp_path: Path) -> None:
    ...
    assert results["numpy"].notes == (
        "non-local query embeddings require scoped ProviderExecutionPolicy",
    )
...
def test_memory_embedding_limited_estimates_are_not_production_scope(
    ...
    with pytest.raises(RuntimeError, match="memory_embedding_projections"):
        estimate_memory_embedding_build(
            db_path,
            provider="gemini",
            dimensions=768,
...
def test_memory_objective_execute_records_no_spend_media_trace(tmp_path: Path) -> None:
    ...
    assert execution.metadata["provider_policy_required"] is True
...
def test_memory_objective_execute_records_provider_policy_external_gap(tmp_path: Path) -> None:
    db_path = tmp_path / "x.sqlite3"
    _seed_db(db_path)
    build_memory_corpus(db_path)
    build_memory_relations(db_path)
    ...
    assert any(
        gap["gap_id"] == "provider_execution_policy_required"
        for gap in execution.metadata["evidence_gap"]["gaps"]
    )
...
def test_memory_objective_execute_skips_provider_freshness_fallbacks(tmp_path: Path) -> None:
    ...
    assert external.output["reason"] == "provider_policy_required"
...
def test_memory_ocr_blocks_provider_quota_calls(tmp_path: Path) -> None:
    ...
    with pytest.raises(
        RuntimeError,
        match="provider OCR API use requires ProviderExecutionPolicy",
    ):
...
def test_memory_objective_routes_and_ocr_cli(tmp_path: Path, capsys) -> None:
    ...
    assert '"provider_policy_required": true' in execution_output
    ...
    assert "provider execution policy is required" in blocked_output.err
...
def test_memory_final_skeleton_preflight_writes_no_spend_contracts(tmp_path: Path) -> None:
    ...
    assert report.provider_policy_required is True
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        assert json.loads(projection[1])["provider_policy_required"] is True
...
def test_memory_final_skeleton_preflight_cli_json_no_store(
    ...
    assert payload["provider_policy_required"] is True
...
def test_memory_final_skeleton_preflight_cli_uses_default_query(
    tmp_path: Path,
    capsys,
) -> None:
    db_path = tmp_path / "x.sqlite3"
    ...
    payload = json.loads(capsys.readouterr().out)
    assert payload["query"] == "final skeleton preflight"
    assert payload["stored"] is True
    assert payload["provider_policy_required"] is True
    assert payload["next_paid_gate"].startswith("provider-backed")
...
def test_memory_rerank_provider_requires_provider_policy(monkeypatch) -> None:
    called = False
...
def test_memory_audit_strict_can_gate_local_no_provider_readiness(tmp_path: Path) -> None:
    db_path = tmp_path / "x.sqlite3"
    _seed_db(db_path)
    build_memory_corpus(db_path)
    build_memory_relations(db_path)
...
def test_memory_audit_marks_openai_compatible_embeddings_provider_policy_required(
    tmp_path: Path,
) -> None:
    db_path = tmp_path / "x.sqlite3"
    _seed_db(db_path)
    ...
    assert any(
        "provider embedding rows require ProviderExecutionPolicy" in item
...
def test_memory_answer_stores_answer_artifact_and_citations(tmp_path: Path) -> None:
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        answer_tool_calls = conn.execute(
            ...
            SELECT COUNT(*)
            FROM memory_tool_calls
            WHERE provider_role = 'answer' AND action = 'answer'
            """
        ).fetchone()[0]
...
def test_memory_relevance_support_fixture_lane_is_deterministic() -> None:
    ...
    assert payload["metadata"]["provider_policy"] == "provider_execution_policy_required"
...
def test_memory_answer_provider_requires_provider_policy(
    tmp_path: Path,
    monkeypatch,
) -> None:
    db_path = tmp_path / "x.sqlite3"
...
def test_llm_context_provider_requires_provider_policy(
    tmp_path: Path,
    monkeypatch,
) -> None:
    called = False
