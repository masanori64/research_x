...
| S34 | `pdgkit` | https://github.com/shibayamalicht/pdgkit, https://note.com/nonoonenono/n/n0701699bbf3c, and npm `@shibayama/pdgkit` | Pinned local pdgkit tool lane for item 35 deterministic diagram/spec rendering and item 34's explanatory note. MIT license and npm version `0.1.2` checked 2026-06-24. Package lock is isolated under `tools/pdgkit_canary/`; real `pdgkit validate` and SVG `pdgkit render` passed for `canaries/item-11-35-flow.pdg`, with deterministic SHA-256 `1023BB0031CC07D94150CCCAD13771A440C7F2094A430770E50FA65A4CB8B623`. Local check also confirmed render lanes for SVG/PNG/JPEG/PDF/PPTX/editable PPTX and `refs` Markdown/CSV through the pinned package. Decision: local `.pdg -> validate -> render/refs` review or presentation artifact lane, with `.pdg` as source of truth. No `pdgkit-mcp`, provider use, root dependency, global install, hook/plugin adoption, or evidence promotion. |
